...
- `Inspect → Extract → Translate → Implement → Verify` と 2D design scope は project identity として維持する。
- individual Skill は standalone installability と reference provenance を保つ。
- repository-specific language policy (internal Skill/docs Japanese; GitHub Issue/PR and commits English) remains an explicit project decision.
