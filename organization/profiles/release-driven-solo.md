...
design-skills の current release-driven development で使用する Operating Model を定義します。
...
- `skills/<name>/SKILL.md` in this repository are product artifacts authored here, not copies to be overwritten by project-init updates.
- project-init Skills used to develop this repository must live in the CLI-managed project-local agent-skill area and remain distinct from the repository's distributable design Skills.
- Inspect → Extract → Translate → Implement → Verify remains the product-specific design workflow.

## Agent Skills lifecycle

- project-local install/reconcile: `bunx skills add rebuildup/project-init --skill '*' --agent claude-code opencode codex -y`
- fresh clone restore: `bunx skills install`
- continuous update: `bunx skills update -p -y`
- commit CLI-generated `skills-lock.json`; do not edit upstream-managed Skill files by hand
